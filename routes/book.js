...
router.get("/ordered/:id", function(req, res) {
  ...
  todo
    .then(input => {
      // var result = lockpay.cancel(user_address, "123456", input[0].contract_address);
      // console.log(result)

      var entry = lockpay.contractEntry(input[0].contract_address);
...
router.post("/orderaction", function(req, res) {
  ...
  var result = lockpay.book(user_address, user_password, contract_address);
  console.log(result);
  ...
  // res.json(data)
  // var entry = lockpay.contractEntry(contract_address);
...
router.get("/list", (req, res) => {
  ...
  var todo = doquery(
    "select *, book.start_date as start_datev2, book.end_date as end_datev2 from items inner join book on items.id = book.item_id where items.status = '1' and book.status = '1' and book.user_id = " +
      user_id,
    ""
  );
...
router.get("/cancel", (req, res) => {
  ...
  var result = lockpay.cancel(user_address, password, contract_address);
  // var result = "here"
  // console.log(result);
  // res.send(result);
  var todo = doquery(
    "update book set status = '0' where user_address = ? and contract_address = ?",
...
router.get("/cancelspecial", (req, res) => {
  var user_address = "0xd290dfe6e650029b661aaa02a644155210a58e96";
  var user_password = "123456";
  var result = lockpay.cancel(
    user_address,
    user_password,
    "0xadfa0f24b014525cd586e89ecaeb0ab25dfa9f65"
  );
  console.log(result);
